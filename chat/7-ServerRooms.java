...
class ClientHandler extends Thread {
    ...
    try {
      ...
      while ((received = in.readLine()) != null) {
        switch (received.split(" ")[0]) {
          ...
          case "/join":
            if (received.split(" ").length > 1) {
              send("joining " + received.split(" ")[1]);
              room = received.split(" ")[1];
            }
            break;
          case "/quit":
            socket.close();
  ...
  public void broadcast(String message, boolean self) {
    for (ClientHandler connection : ServerRooms.connections) {
      if ((connection != this || self) && connection.room.equals(this.room)) {
